...
//    Expr x_clamped("x_clamped") = clamp(x, 0, input.width()-1);
//    Expr y_clamped("y_clamped") = clamp(y, 0, input.height()-1);

    // sqrt version does well with preserving color.
    // remove sqrt if so desire.
    clamped(x, y, c) = cast<float>(sqrt(input(clamp(x, 0, input.width()-1), clamp(y, 0, input.height()-1), c)/255.0f));
    ...
    RDom box_wimg(0, input.width()-1, 0, input.height()-1,0,2);
    // TODO: this is a simplest but very rough estimation for airligh.
    // fix it using argmax as mentioned above
//    Func airlight("airlight");
//    airlight(c) = maximum(clamped(0+box_wimg.x, 0+box_wimg.y, c));
//    Tuple maxValue= argmax(clamped(0+box_wimg.x, 0+box_wimg.y, 0+box_wimg.z));
    Expr airlight("airlight");

    airlight = maximum(clamped(0+box_wimg.x, 0+box_wimg.y, 0+box_wimg.z));
    // filter out airlight
    Func clamped_f_no_air("clamped_f_no_air"), dcpf_p("dcpf_p");
    clamped_f_no_air(x, y, c) = clamped(x, y, c)/airlight;
    ...
    Func  mu_wnr("mu_wnr"); // for finding mu
    mu_wnr(x, y)=sum(dcpf_p(x + box_wnr.x, y + box_wnr.y))/(float)(kernel_size*kernel_size);
    Func dcpf2("dcpf2"), sigma2_wnr("sigma2_wnr"); // for calculating sigma2
    dcpf2(x, y) = dcpf_p(x, y)*dcpf_p(x, y);
    sigma2_wnr(x, y) = max(sum(dcpf2(x + box_wnr.x, y + box_wnr.y))/(float)(kernel_size*kernel_size) - mu_wnr(x, y)*mu_wnr(x, y), 0.0f);



    // begin to filter DCP (dcpf_p) with adaptive wiener filter
    //TODO: is this a practical implementation to find sigma2_n?
    Func sigma2_n("sigma2_n");
    Var dummy("dummy");
    sigma2_n(dummy) = 0.0f;
    ...
    Func dcpf_refine("dcpf_refine");
    dcpf_refine(x, y) = mu_wnr(x, y) + weight(x, y)*(dcpf_p(x, y)-mu_wnr(x, y));

    // get transmission map
    Expr t_map("t_map");
    t_map = clamp(1.0f-0.95f*dcpf_refine(x, y), 0.01f, 1.0f);

    // recover image
    Func restore_img("restore_img");
    restore_img(x, y, c) = cast<uint8_t>(clamp((clamped(x, y, c)-airlight)/t_map + airlight, 0.0f, 1.0f)*255.0f);
    AUTOTUNE_HOOK(restore_img);
