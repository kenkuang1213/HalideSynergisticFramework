...
//            std::cout<<"GPU"<<std::endl;
		...
		Var yi;
		output.parallel(y, 8).vectorize(x, 8);
		gray.compute_root().parallel(y, 8).vectorize(x, 8);
		for (int j = 0; j < 4; j++) {
			if (j > 0) inGPyramid[j].compute_root().parallel(y, 8).vectorize(x, 8);
			if (j > 0) gPyramid[j].compute_root().parallel(y, 8).vectorize(x, 8);
			outGPyramid[j].compute_root().parallel(y, 8).vectorize(x, 8);
		}
		for (int j = 4; j < J; j++) {
