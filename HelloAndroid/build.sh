...
cd jni
c++ gaussinBlur.cpp -L $HALIDE_PATH/bin -lHalide -I $HALIDE_PATH/include -ldl -lpthread -lz
...
for archs in arm-32-android,armeabi arm-32-android-armv7s,armeabi-v7a ; do
    ...
    HL_TARGET=$hl_target DYLD_LIBRARY_PATH=$HALIDE_PATH/bin LD_LIBRARY_PATH=$HALIDE_PATH/bin:$LD_LIBRARY_PATH ../a.out
	 HL_TARGET=$hl_target-opencl DYLD_LIBRARY_PATH=$HALIDE_PATH/bin LD_LIBRARY_PATH=$HALIDE_PATH/bin:$LD_LIBRARY_PATH ../a.out
    cd ..
    unset IFS
done
